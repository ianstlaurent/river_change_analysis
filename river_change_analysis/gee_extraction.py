...
def process_images(start_year, end_year, month_day_start, month_day_end, roi, folder_name, file_name):
    ...
    for year in range(start_year, end_year+1):
        ...
        watermask_jrc = jrcYearly.filter(ee.Filter.eq('year', year)).first() \
        .gte(2).unmask(0) \
        .clip(roi)
        ...
        source= watermask.And(watermask_jrc.ee.Image().byte().paint(sword, 1)),
        maxDistance=3E3,
        geodeticDistance=False
        )

        wetted_rivermask = costmap.eq(0).rename('riverMask')
        wetted_rivermask= wetted_rivermask.And(watermask)
